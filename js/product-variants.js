class ProductVariantDropdown extends HTMLElement {
  ...
  updateMedia() {
    ...
    const $FeaturedImage = $('.FeaturedImage_slick');
    if ($FeaturedImage.length && typeof $FeaturedImage.slick === 'function' && $FeaturedImage.hasClass('slick-initialized')) {
      const dindex = $(newMedia).attr('data-slick-index') || $(newMedia).data('position');
  ...
  renderProductInfo() {
    ...
    fetch(`${this.dataset.productUrl}?variant=${requestedVariantId}&section_id=${this.dataset.originalSection ? this.dataset.originalSection : this.dataset.productId}`)
      ...
      .then((responseText) => {
        ...
        const quantity = document.getElementById("productSelect").querySelector('[value="' + variant_id + '"]').getAttribute('data-qty');
        ...
        if (typeof maxQuantity !== 'undefined' && maxQuantity) {
          maxQuantity.setAttribute('max', quantity);
        }
      });
  }
